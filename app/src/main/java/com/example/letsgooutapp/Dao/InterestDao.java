...
public interface InterestDao {
    ...
    @Query("SELECT eventId FROM interest_table WHERE interest = :interest")
    LiveData<List<Integer>> getEventIdsByInterest(String interest);
            // <INTEGER> MAYBE ?!!?!
    @Query("SELECT interest FROM interest_table WHERE eventId = :eventId")
    LiveData<List<String>> getInterestsByEventId(int eventId);
            //String ?!?!?!?
